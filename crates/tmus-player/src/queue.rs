...
impl Queue {
    ...
    /// Сколько позиций порядка обхода осталось после курсора. Ноль
    /// аллокаций: при shuffle это `order.len() - cursor - 1`, при
    /// выключенном — позиция в векторе совпадает с порядком обхода.
    /// Курсора нет (нечего обходить) — считается, что позиций не осталось.
    #[must_use]
    pub fn positions_remaining(&self) -> usize {
        match self.cursor {
            Some(cursor) => self.order_len().saturating_sub(cursor + 1),
            None => 0,
        }
    }
